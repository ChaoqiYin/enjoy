...
use crate::language::{resolve, LanguageState};
...
static ENGLISH: LazyLock<Value> = LazyLock::new(|| {
    serde_json::from_str(include_str!("../../src/locales/en/native.json"))
        .expect("Embedded English translations are valid")
});
static CHINESE: LazyLock<Value> = LazyLock::new(|| {
    serde_json::from_str(include_str!("../../src/locales/zh-CN/native.json"))
