...
const video: Video = {
  id: 1, path: '/movies/example.mp4', file_name: 'example.mp4',
  folder_path: '/movies', file_size: 1024, modified_at: 0,
  duration_ms: 65000, width: 1920, height: 1080, codec: 'h264',
  thumbnail_path: null, favorite: false, available: true,
  play_count: 3, last_played_at: 10, created_at: 0, updated_at: 0,
};
const actions = () => ({
...
function view(handlers = actions(), onClose = vi.fn(), busy = false) {
  return <I18nextProvider i18n={i18n}><VideoDetails video={video} actions={handlers} busy={busy} onClose={onClose} /></I18nextProvider>;
}
...
it('closes with Escape or the backdrop and confines keyboard focus', () => {
  ...
  fireEvent.click(screen.getAllByRole('button', { name: english.closeDetails })[0]);
  expect(close).toHaveBeenCalledTimes(2);
});
it('reports maintenance failure and retries without losing existing details', async () => {
  const handlers = actions();
  handlers.refreshInfo.mockRejectedValueOnce(new Error('failed')).mockResolvedValue(undefined);
  render(view(handlers));
  await act(async () => fireEvent.click(screen.getByRole('button', { name: english.refreshInfo })));
  expect(screen.getByRole('alert')).toBeTruthy();
  expect(screen.getByText('example.mp4')).toBeTruthy();
  await act(async () => fireEvent.click(screen.getByRole('button', { name: english.retry })));
